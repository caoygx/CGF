...
//use Common\Cgf\CgfInterface; implements CgfInterface
class CgfController //extends CommonController implements FrameworkInterface
{
    ...
    public $lockedDefinitions = [
        'express', 'goods', 'goods_activity', 'log_request',
    ];
    ...
    public function __construct($pre = '', $modelName = '')
    {
        ...
        parent::__construct($pre, $modelName);
            ...
            ///$tableName = str_replace(config('DB_PREFIX'),'',$tableName);
            $formHandler      = new BootstrapForm();
            $tableInfoHandler = new MysqlTableInfo(null, $tableName);
            $this->cgf        = new Cgf($tableName, config('DB_PREFIX'), $this, $formHandler, $tableInfoHandler);
    }


    protected $dbConnection = [];

    protected function getControllerTitle($controllerName)
    {
        $menu = config('menu');
        //旧版
        /*
        foreach ($menu as $k => $v) {
            if ($v['controller'] == $controllerName ) return $v['title'];
        }*/

        //新版
        /*$allMenu = [];
        foreach ($menu as $k=>$v){
            if($v['children']) array_merge($allMenu,$v['children']);
        }

        foreach ($allMenu as $k => $v) {
            if ($v['controller'] == $controllerName || $v['url'] == $controllerName) return $v['title'];
        }*/
        ...
        foreach($menu as $k=>$v){
            foreach($v['children'] as $k2=>$v2){
                if(strtolower($v2['url']) == strtolower($currentUrl)) return $v2['title'];
            }
        }
    }
...
//var_dump($r);exit;
    ...
    function generateListTitle()
    {
        ...
        foreach ($this->cgf->definition->list as $k => $v) {
            ...
            if (!empty($v['js_function'])) {
                ...
                $js_function  = "{$this->cgf->tableName}_$functionName";
     ...
     * @return string
     */
    function generateListTpl()
    {

        $str = $this->_generateListsTemplate();
     ...
     * @return string
     */
    function _generateListsTemplate()
    {
        $arrTplParameter = [];
        ...
        $f_action   = $this->cgf->definition->getAllColumnDefinition()['action'];
        $pageButton = $this->cgf->definition->getAllColumnDefinition()['pageButton'];
        ...
        //action操作生成
        if (empty($f_action)) {
            $fieldsKey  = 'tpl_fields.' . strtolower($this->cgf->tableName);
            $tpl_fields = config($fieldsKey);
        ...
        /*if(empty($tpl_fields['f_action'])){
            //$this->f_action = config('f_action');
            $arrTplParameter['f_action'] =config('f_action');
        }else{
        ...
        //搜索框生成
        //$this->page = 'search';
        $htmlSearch = $this->cgf->generateSearch();
        ...
        $file    = APP_PATH . MODULE_NAME . "/View/Public/tpl_list" . config('TMPL_TEMPLATE_SUFFIX');
        if(!file_exists($file)){
    ...
    function generateListTplToFile()
    {
        ...
        $listFields                = $this->cgf->generateListTitle();
        ...
        $listTitle = $this->cgf->generateListTitle();
        ...
        $tplPath  = THEME_PATH . $template . config('TMPL_TEMPLATE_SUFFIX');
...
//exit;
    ...
    {
        $htmlAdd = $this->cgf->generateAdd($isEdit);
        ...
        $file    = APP_PATH . MODULE_NAME . "/View/Public/tpl_add" . config('TMPL_TEMPLATE_SUFFIX');
        if(!file_exists($file)){
            exit('no exist tpl_add');
        }
        $content = file_get_contents($file);
    ...
    function realExportExcel($list)
    {
        ...
        $xlsName = $this->cgf->definition->getAllColumnDefinition()['title'];
    ...
    {
        config('URL_MODEL', 0); //解决时间搜索中 空格被转成+号，导致下一页内容无法显示
